...
Để hiển thị số ra màn hình cho con người đọc được, máy tính thực tế luôn phải thực hiện một bước chuyển đổi từ giá trị nhị phân sang các mã ký tự tương ứng. Hàm itoa_custom chính là cách tường minh hóa quy trình này để hiểu sâu hơn về cách máy tính xử lý dữ liệu
 * @Return: void
 */
...
int main() {
    int n;
    char s[20]; // Số nguyên int thường tối đa 11 ký tự (kể cả dấu -), 20 là dư dùng.

    printf("Nhap vao mot so nguyen: ");
    
    // Sử dụng scanf để nhập số nguyên
    if (scanf("%d", &n) == 1) {
        
        // Gọi hàm itoa_custom để chuyển đổi số n thành chuỗi s
        itoa_custom(n, s);
        
        // In kết quả ra màn hình để kiểm tra
        printf("Chuoi sau khi chuyen doi (ASCII): %s\n", s);
    } else {
